 ...
 *
 * หกหน้าย่อยใช้ข้อมูลชุดเดียวกัน (บิล + รายการสินค้า + รายจ่าย ของช่วงวันที่ที่เลือก)
 * จึงโหลดครั้งเดียวแล้วสลับหน้าได้เลย ไม่ต้องยิงใหม่ทุกครั้ง — index.js เรนเดอร์คอมโพเนนต์นี้
 * ตัวเดียวสำหรับทั้ง 6 เมนู (เปลี่ยนแค่ prop view) state ที่โหลดไว้จึงไม่หายตอนสลับเมนู
 *
...
/* ── สีของกราฟ: โทนเขียวตาม CI ของ GOODFOOD ── */
...
/* ประเภทออร์เดอร์ — ฐาน Aoringo เก็บเป็นข้อความใน SaleOrder.OrderType ซึ่งแต่ละที่เขียนไม่เหมือนกัน
   ...
   อะไรที่ไม่ใช่ "กลับบ้าน" หรือ "เดลิเวอรี" นับเป็นทานที่ร้าน */
...
/* คอลัมน์ตารางยอดขายรายวัน
   ช่องทางจ่ายไม่ได้ fix ไว้ตายตัว — เติมตามช่องที่ฐานนั้นมียอดจริง (ดู usedChannels) */
...
export default function Franchise({ view = 'fcDashboard' }) {
  ...
  /* ── หน้าย่อย: รายงานยอดขาย (รายบิล + ดูบิล + ประวัติโต๊ะ) ──
     ปุ่มดูบิลอยู่หน้าสุดของแต่ละแถว
  ...
  return (
    <div className="space-y-6">
      ...
      {/* บิลเบื้องหลังตัวเลขที่กดในตารางรายวัน */}
      {drill.open && (
        <div
