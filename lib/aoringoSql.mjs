...
/* ══════════════════════════════════════════════════════════════════════
   ...
   ข้างบนจึงใช้ไม่ได้ (เคยเดาไปหยิบ dbo.Expense มาเป็นตารางบิล):
     ...
     · ความเคลื่อนไหวของออเดอร์ (เปิด/สั่งเพิ่ม/ยกเลิก/ชำระเงิน) อยู่ที่ OrderActivity
       ซึ่งก็ไม่มีวันที่ของบิลในตัวเอง ต้อง join SaleOrder เหมือนกัน

   ทุกคอลัมน์ยังเช็กก่อนใช้ว่ามีจริงไหม — แอปเวอร์ชันหน้าเพิ่ม/ตัดคอลัมน์แล้วต้องไม่พังทั้งหน้า
   ══════════════════════════════════════════════════════════════════════ */

/* ชื่อคอลัมน์ "สาขา" ที่เป็นไปได้ — โครงแอปแต่ละเวอร์ชันเรียกไม่เหมือนกัน
   เรียงเอาชื่อที่อ่านรู้เรื่อง (…Name) ขึ้นก่อนรหัส เพราะค่านี้ไปโผล่บนหน้าเว็บตรง ๆ
   ไม่มีสักตัว = คืน NULL เหมือนเดิม หน้าเว็บจะขึ้นว่าฐานนี้ไม่มีข้อมูลสาขา ไม่พังทั้งหน้า */
const BRANCH_COLS = [
  'BranchName', 'StoreName', 'ShopName', 'OutletName',
  'BranchCode', 'StoreCode', 'ShopCode', 'OutletCode',
  'BranchId', 'StoreId', 'ShopId', 'OutletId',
...
/* ชื่อคอลัมน์ที่เป็นไปได้ของตาราง OrderActivity — แอปแต่ละเวอร์ชันเรียกไม่เหมือนกัน
   ตัวแรกที่มีอยู่จริงในตารางคือตัวที่ถูกเลือก ไม่มีสักตัวก็ปล่อยเป็น NULL ไม่ล้มทั้งหน้า */
const ACTIVITY_FK    = ['OrderId', 'SaleOrderId'];
const ACTIVITY_AT    = ['ActivityAt', 'CreatedAt', 'ActivityDate', 'OccurredAt', 'LoggedAt', 'ActivityTime', 'CreateDate'];
const ACTIVITY_TYPE  = ['ActivityType', 'Action', 'ActionType', 'EventType', 'Activity', 'Event', 'Type', 'Status'];
const ACTIVITY_TEXT  = ['Description', 'Detail', 'Details', 'Message', 'Note', 'Remark', 'Content', 'Data'];
// FK ของคนทำรายการ — เอาเฉพาะชื่อที่เป็น "รหัสผู้ใช้" จริง ๆ ถ้าไปหยิบคอลัมน์ที่เก็บ "ชื่อคน"
// มา join กับ UserId ที่เป็นตัวเลข SQL Server จะฟ้อง conversion error ทั้งคำสั่ง
const ACTIVITY_USER_FK = ['UserId', 'AppUserId', 'CreatedByUserId', 'PerformedByUserId', 'ActorUserId', 'ByUserId'];
...
export function detectAppSchema(tables) {
  ...
  return {
    ...
    activity: table1(tables, 'OrderActivity') || table1(tables, 'SaleOrderActivity') ||
      table1(tables, 'OrderActivityLog') || table1(tables, 'OrderLog'),
  };
}
...
/** ชื่อคน — join ตารางผู้ใช้จากคอลัมน์ FK ของตารางไหนก็ได้ (ชื่อคอลัมน์ต่างเวอร์ชันกันได้)
    ไม่มีตารางผู้ใช้/ไม่มี FK/โครงตารางผู้ใช้ไม่ตรง = ไม่ join คืน null ให้คนเรียกถอยไปทางอื่น */
function userJoin(app, from, fromAlias, fkNames, joinAlias) {
  const u = app.users;
  if (!u || !from) return null;
  const fk = fkNames.map((n) => ref(from, fromAlias, n)).find(Boolean);
  if (!fk) return null;
  ...
  return {
    sql: `LEFT JOIN ${u.full} ${joinAlias} ON ${joinAlias}.${bracket(pk)} = ${fk}`,
    expr: `${joinAlias}.${bracket(nameCol)}`,
  };
}

/** ชื่อคนปิดบิล (ปิดไม่ได้ก็เอาคนเปิดออร์เดอร์) */
const cashierJoin = (app) => userJoin(app, app.orders, 'o', ['PaidByUserId', 'OpenedByUserId'], 'u');
...
export function buildAppPlan(app) {
  ...
  const billCols = [
    ...
    pickAs(o, 'o', BRANCH_COLS, 'outletId'),
  ...
  /* ── ประวัติออเดอร์ (OrderActivity) ──
     ตารางบันทึกความเคลื่อนไหวรายออเดอร์ (เปิดโต๊ะ / สั่งเพิ่ม / ย้ายโต๊ะ / ยกเลิก / ชำระเงิน …)
     ไม่มีวันที่ของบิลในตัวเอง จึง join SaleOrder แล้วกรองด้วย "วันที่ของบิล" ชุดเดียวกับตารางบิล
     — ช่วงที่โหลดมาจะได้ตรงกันเป๊ะ ทุกบิลบนหน้าจอมีประวัติของตัวเองครบ ไม่ตกหล่นที่หัว/ท้ายช่วง */
  let activity = null;
  const ac = app.activity;
  const acFk = ac && ACTIVITY_FK.map((n) => ref(ac, 'a', n)).find(Boolean);
  if (ac && acFk) {
    const acAt = ACTIVITY_AT.map((n) => ref(ac, 'a', n)).find(Boolean);
    const acUser = userJoin(app, ac, 'a', ACTIVITY_USER_FK, 'au');
    const actCols = [
      // เวลาที่เกิดเหตุ — ตารางไหนไม่เก็บเวลาของตัวเองก็ใช้เวลาของบิลไปก่อน จะได้เรียงไทม์ไลน์ได้
      `${acAt || d.expr} AS ${bracket('date')}`,
      `${d.expr} AS ${bracket('orderDate')}`,     // ไว้จับคู่กับบิลบนหน้าเว็บ (วันที่+เลขที่บิล)
      pickAs(o, 'o', ['OrderNo', 'OrderId'], 'checkId'),
      `${acFk} AS ${bracket('orderId')}`,
      pickAs(o, 'o', ['TableName', 'TableId'], 'tableId'),
      pickAs(ac, 'a', ACTIVITY_TYPE, 'activityType'),
      pickAs(ac, 'a', ACTIVITY_TEXT, 'detail'),
      acUser ? `${acUser.expr} AS ${bracket('user')}` : pickAs(ac, 'a', ['UserName', 'CreatedBy', 'User'], 'user'),
      pickAs(ac, 'a', ['MenuName', 'ItemName', 'ProductName'], 'itemName'),
      pickAs(ac, 'a', ['Qty', 'Quantity'], 'quantity'),
      pickAs(ac, 'a', ['Amount', 'Total', 'Value'], 'amount'),
      pickAs(ac, 'a', ['OrderActivityId', 'ActivityId', 'Id'], 'activityId'),
    ];
    activity = {
      table: `${ac.full} + ${o.name}`,
      dateColumn: d.label,
      sql: (top) => `SELECT TOP ${top}
             ${actCols.join(',\n             ')}
        FROM ${ac.full} a
        JOIN ${o.full} o ON ${ref(o, 'o', 'OrderId')} = ${acFk}
        ${acUser ? acUser.sql : ''}
       WHERE ${d.expr} >= @start AND ${d.expr} <= @end
       ORDER BY ${d.expr}${acAt ? `, ${acAt}` : ''}`,
    };
  }
  ...
  if (e && eDate) {
    ...
    if (amountExpr) {
      const expCols = [
        ...
        pickAs(e, 'e', BRANCH_COLS, 'outletId'),
  ...
  return { mode: 'app', bill, item, expense, activity };
}
...
export function createAoringo(db) {
  ...
  async function readLayout({ force = false } = {}) {
        ...
        layoutCache = {
          ...
          roles: {
            ...
            activity: plan.activity
              ? { table: plan.activity.table, dateColumn: plan.activity.dateColumn, sqlPreview: plan.activity.sql(5) }
              : null,
          },
          plan,
          // แนบชื่อคอลัมน์มาด้วย ไม่ใช่แค่จำนวน — เวลาหาว่าฐานนี้เก็บสาขา/ส่วนลด/อะไรไว้คอลัมน์ไหน
          // จะได้ดูจบในคำขอเดียว ไม่ต้องไปไล่ SELECT INFORMATION_SCHEMA ที่เครื่อง SQL เอง
          tables: tables.map((t) => ({ name: t.full, columns: t.columns.length, columnNames: t.columns.map((c) => c.name || c) })),
    ...
    out.tables = tables.map((t) => ({ name: t.full, columns: t.columns.length, columnNames: t.columns.map((c) => c.name || c) }));
    layoutCache = out;
    layoutAt = Date.now();
    return out;
  }
  ...
  /** ประวัติออเดอร์ (OrderActivity) — มีเฉพาะโครงแอปร้านอาหาร ไม่มีตัวจับคู่อัตโนมัติให้
      เพราะตารางแบบ log ไม่มีคอลัมน์ตายตัวพอจะเดาได้ว่าตารางไหนคือ "ประวัติของออเดอร์" */
  async function readActivities(opt) {
    const layout = await readLayout();
    if (layout.mode !== 'app' || !layout.plan.activity) {
      throw new Error(
        'ฐาน Aoringo นี้อ่านประวัติออเดอร์ไม่ได้ — ต้องเป็นโครงแอปร้านอาหารที่มีตาราง OrderActivity ' +
        'ซึ่งอ้างถึงออเดอร์ด้วยคอลัมน์ OrderId (ดูรายชื่อตารางทั้งหมดที่ /api/franchise?view=schema)');
    }
    return runRange('activity', 'ประวัติออเดอร์', opt, 200000);
  }
  ...
  return { readLayout, forgetLayout, readBills, readItems, readExpenses, readActivities, ping };
}
