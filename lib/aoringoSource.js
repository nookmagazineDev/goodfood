...
// ที่อยู่ host API — ตั้ง AORINGO_API_BASE ทับได้ ค่าเริ่มต้นคือ tunnel ของเครื่องออฟฟิศที่ใช้อยู่เดิม
// (ตัวเดียวกับที่ระบบอื่นของร้านเรียก จึงรับ /aoringo/* ได้อยู่แล้ว)
...
async function getFromHost(path, { timeoutMs = HOST_TIMEOUT } = {}) {
  ...
  try { json = JSON.parse(text); }
    ...
    throw new Error(
      ...
      'และเป็นเวอร์ชันที่มี /aoringo/* แล้วหรือยัง (git pull แล้วรีสตาร์ท node server.js)');
  }
