...
| `0004-branch-filter-and-settings.patch` | **แยกสาขา** — ตัวเลือกสาขาในแถบตัวกรอง (กรองฝั่งหน้าเว็บ ข้อมูลที่โหลดไว้ไม่หายตอนสลับ) + หน้าย่อย **ตั้งค่าสาขา** ที่ลิสต์สาขาที่พบในข้อมูลจริง ตั้งชื่อเรียกและติ๊กว่าสาขาไหนเป็น "สาขาเทส" ได้ · สาขาเทสถูกตัดออกจากทุกหน้าโดยอัตโนมัติ เว้นแต่เลือก "ทุกสาขา + สาขาเทส" |
| `0005-detect-branch-column.patch` | ให้ฝั่ง SQL **ตรวจหาคอลัมน์สาขา** ใน `SaleOrder`/`Expense` (ดู `BRANCH_COLS`) แทนที่จะตั้ง `outletId` เป็น `NULL` ตายตัว — ไม่มีคอลัมน์สาขาก็คืน `NULL` เหมือนเดิม ไม่กระทบอะไร |
...
ทุกสาขาอยู่ฐาน `Aoringo` เดียวกัน แยกด้วยคอลัมน์สาขาใน `SaleOrder`/`Expense`
หน้าเว็บโหลดทั้งช่วงวันที่มาครั้งเดียวแล้วกรองฝั่งหน้าเว็บ สลับสาขาจึงไม่ต้องโหลดใหม่

หน้า **ตั้งค่าสาขา** ลิสต์สาขาที่โผล่ในข้อมูลจริง (ไม่ได้ให้เพิ่มสาขาเอง) ตั้งชื่อเรียกและ
ติ๊กว่าสาขาไหนเป็น **สาขาเทส** ได้ — สาขาเทสถูกตัดออกจากทุกหน้าโดยอัตโนมัติ
เว้นแต่เลือก "ทุกสาขา + สาขาเทส" ในแถบตัวกรอง

ค่าที่ตั้งไว้เก็บใน `localStorage` ของเบราว์เซอร์เครื่องนั้น เพราะเมนูนี้อ่านฐานอย่างเดียว
ไม่มีฝั่งเขียนกลับ — เปิดจากอีกเครื่องต้องตั้งใหม่

ถ้าหน้าตั้งค่าสาขาขึ้นว่า "ฐานนี้ยังไม่มีข้อมูลสาขา" แปลว่าตารางบิลไม่มีคอลัมน์สาขา
หรือชื่อไม่ตรงกับที่มองหา — เปิด `/api/franchise?view=schema` ดูชื่อคอลัมน์จริง
แล้วเพิ่มเข้า `BRANCH_COLS` ใน `lib/aoringoSql.mjs`
