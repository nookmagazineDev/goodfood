# ข้อมูลร้านเฟรนไชส์ — ฐาน Aoringo

เว็บนี้มีเมนูเดียวคือ "เฟรนไชส์" (โทนเขียวตาม CI) แบ่งเป็น 6 หน้าย่อย:

| หน้าย่อย | เห็นอะไร |
|---|---|
| แดชบอร์ด | ยอดขายรวม · จำนวนบิล · จำนวนลูกค้า · รายจ่าย · กราฟยอดขายรายวัน · ช่องทางชำระเงิน · เมนูขายดี 10 อันดับ · รายจ่ายตามประเภท |
| รายงานยอดขาย | ตารางรายบิล พร้อมปุ่ม **ดูบิล** (หัวบิล + รายการในบิล + วิธีชำระ) และ **ประวัติโต๊ะ** (ทุกบิลของโต๊ะนั้นในช่วงที่เลือก) |
| ยอดขายรายวัน | Dine-in/Take-Home/Delivery · Service Charge · ส่วนลด · Net/Vat/Gross · แยกช่องทางชำระ · Total Sales · บิล/หัว/เฉลี่ย · รายจ่ายของวันนั้น (กดหัวคอลัมน์เพื่อเรียง · กดตัวเลขเพื่อดูบิลเบื้องหลัง) |
...
ฐานข้อมูล **`Aoringo`** บน SQL Server (ค่าเริ่มต้น `203.154.185.48`) ทางไปถึงฐานมีสองทาง
ลองตามลำดับนี้:

1. **ต่อ SQL ตรงจากเซิร์ฟเวอร์เว็บ** — `lib/aoringoPool.js` เร็วที่สุด ใช้ได้เมื่อพอร์ตเปิดให้ IP ของเว็บ
...
ฐาน Aoringo ไม่ใช่โครง POS แบบ NaraiPos แต่เป็นแอปร้านอาหารที่กระจายข้อมูลหลายตาราง
...
ฐาน Aoringo เป็นของร้านเฟรนไชส์ ไม่มีใครรับประกันว่าชื่อตาราง/คอลัมน์จะตรงกัน `lib/aoringoSql.mjs`
จึงอ่าน `INFORMATION_SCHEMA` ตอนรัน แล้วให้คะแนนหาตารางที่ "หน้าตาเหมือน" 3 บทบาท
(บิล · รายการสินค้า · รายจ่าย) โดยดูที่คอลัมน์เป็นหลัก ชื่อตารางเป็นแค่ตัวช่วย
จากนั้นจับคู่คอลัมน์ตามรายการชื่อที่เป็นไปได้ ช่องไหนไม่มีก็ปล่อยว่าง
...
เดาผิดให้ตั้งทับด้วย env (ตั้งที่เว็บ และ/หรือเครื่องโฮสต์):
...
### ฝั่งเว็บ (ต่อ SQL ตรง)

| env | ค่าเริ่มต้น |
|---|---|
| `AORINGO_DB_HOST` | `203.154.185.48` (ไม่ตั้ง → ลอง `QCRD_DB_HOST` / `HR_DB_HOST` / `ZK_DB_HOST` ก่อน) |
...
ไม่ตั้ง `AORINGO_DB_USER`/`AORINGO_DB_PASSWORD` เลยก็ได้ — จะเหลือแค่ทาง host API

⚠️ login ต้องมีสิทธิ์ใน **ฐาน Aoringo** ไม่งั้นต่อติดแต่ query ไม่ผ่าน ("is not able to access the database")
ที่เครื่อง SQL สั่งครั้งเดียว:

```sql
USE Aoringo;
CREATE USER [<login>] FOR LOGIN [<login>];
ALTER ROLE db_datareader ADD MEMBER [<login>];   -- เว็บนี้อ่านอย่างเดียว ไม่ต้องให้สิทธิ์เขียน
```

### เครื่องออฟฟิศ (host API)

ดู `host-server/README.md`

## endpoint ที่มี

| ทาง | endpoint |
|---|---|
| เว็บ | `/api/franchise?view=all\|sales\|detail\|expense\|schema\|diag&start=YYYY-MM-DD&end=YYYY-MM-DD` |
| host | `/aoringo/ping` · `/aoringo/schema` · `/aoringo/sales` · `/aoringo/detail` · `/aoringo/expense` |

เว็บนี้ **อ่านอย่างเดียว** ไม่มีฝั่งเขียน จึงไม่ต้องมีกุญแจ (`x-api-key`)
